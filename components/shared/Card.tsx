...
const Card = async ({
  ...
  return (
    <>
      <div
        className="group relative flex min-h-[150px] sm:min-h-[320px] w-full sm:max-w-[400px] sm:flex-col gap-2 overflow-hidden rounded-sm transition-all md:min-h-[350px] backdrop-blur-sm"
        // style={{
        //   backgroundImage: `url(${event.imageUrl})`,
        //   // backgroundPosition: "center",
        // }}
      >
        <div className="relative h-full w-1/2 sm:h-full sm:w-full overflow-hidden ">
          <Link
        ...
        {/* INFO FOR MOBILE */}
        <div className="sm:hidden flex-row w-1/2 gap-2 rounded-sm bg-dark/50 backdrop-blur-lg shadow-xl  p-2">
          <div className="flex flex-col gap-2">
            ...
            <div className="w-full sm:hidden flex gap-3">
              {!hidePrice && (
                <div className="h-full w-full flex gap-2">
                  <p className="p-semibold-14 w-full rounded-sm bg-primary-500 text-white dark:bg-dark-500 dark:text-white py-1 line-clamp-1 text-center">
                    {event.isFree ? "Gratuit" : `${event.price}€`}
                  </p>
                  <p className="text-center p-semibold-14 w-full rounded-sm bg-grey-500/10 py-1 text-grey-500 line-clamp-1">
                    {event.Category?.name}
                  </p>
                </div>
              )}
        ...
        {/* DIV INFOS EVENT FOR TABLET & PC */}
        <div className="hidden sm:flex flex-row justify-between rounded-b-sm px-2 py-1 backdrop-blur-lg shadow-xl">
          <div className="flex flex-col gap-2">
            {/* TITRE */}
            <Link href={`/events/${event.id}`}>
              <p className="p-medium-16 md:p-medium-20 line-clamp-2 flex-1 text-black">
      ...
      {/* <div className="sm:hidden flex flex-col gap-3 md:gap-4">
        {!hidePrice && (
          <div className="h-full flex flex-col justify-center gap-2">
            <p className="p-semibold-14 w-full rounded-sm bg-primary-500 text-white dark:bg-dark-500 dark:text-white px-4 py-1 line-clamp-1 text-center">
              {event.isFree ? "Gratuit" : `${event.price}€`}
